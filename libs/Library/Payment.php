...
class Payment
{
	...
	{
		switch ($payment_id){
			case self::ALIPAY_DIRECT : {//支付宝即时到帐
				return new \Library\payment\pay_direct_alipay\direct_alipay($payment_id);
			}
			break;
			case self::UNIONPAY : {//银联在线支付
				return new \Library\payment\pay_unionpay\unionpay($payment_id);
