	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		{foreach:items=$data['list']}
			<tr class="text-c">
				...
				<td>{if:$item['jiesuan_prove']}已确认{else:}未确认{/if}</td>
				<td class="td-manage">
				 <a title="详情" href="{url:/balance/accmanage/jiesuanprovedetail}id/{$item['id']}" class="ml-5" style="text-decoration:none">详情</a></td>
			</tr>
		{/foreach}
		</tbody>
