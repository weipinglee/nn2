        ...
        <tbody>
            {foreach:items=$data}
                <tr class="text-c">
                    ...
                    <td>{$item['mode_txt']}</td>
                     <td class="td-manage"> <a title="查看" href="{url:/trade/OfferManage/offerDetails}id/{$item['id']}/user/{$item['username']}" class="ml-5" style="text-decoration:none"><i class="icon-eye-open"></i></a> <a title="删除" ajax_status=-1 ajax_url="{url:/trade/OfferManage/setStatus}id/{$item['id']}"  class="ml-5" style="text-decoration:none"><i class="icon-trash"></i></a></td>
                </tr>
           {/foreach}
        </tbody>
    </table>
