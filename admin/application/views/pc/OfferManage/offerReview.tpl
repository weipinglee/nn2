            ...
            {foreach:items=$data}
                <tr class="text-c">
                    ...
                    <td>{if:$item['mode']==1}自由{elseif:$item['mode']==2}保证金{elseif:$item['mode']==3}仓单{/if}</td>
                     <td class="td-manage"> <a title="查看" href="{url:/trade/OfferManage/reviewDetails}id/{$item['id']}/user/{$item['username']}" class="ml-5" style="text-decoration:none"><i class="icon-eye-open"></i></a> <a title="删除" href="javascript:;" ajax_status=-1 ajax_url="{url:/trade/OfferManage/setStatus}id/{$item['id']}"    class="ml-5" style="text-decoration:none"><i class="icon-trash"></i></a></td>
                </tr>
           {/foreach}
