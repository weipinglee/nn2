...
class Order{
			...
			if($check_payment){
				...
				switch ($orderData['payment']) {
					...
					case self::PAYMENT_BANK:
						//银行签约账户
						$balance = $this->bank->getActive($user_id);
						break;
	...
	public function buyerRetainage($order_id,$user_id,$payment='online',$proof = '',$account=0){
		...
		if(is_array($info) && isset($info['contract_status'])){
			...
			if($info['contract_status'] == self::CONTRACT_BUYER_RETAINAGE || $info['contract_status'] == self::CONTRACT_NOTFORM){
				...
				$sim_oper = in_array($info['mode'],array(self::ORDER_ENTRUST));
				if($retainage>0){
					try {
						...
						if($payment == 'online'){
							...
							if($upd_res['success'] == 1){
								...
								$content =  '(合同'.$info['order_no'].',买方已支付尾款,请您及时进行凭证确认,并关注资金动态。)'.$jump_url;
		...
		if($retainage > 0){
			if($info['seller_deposit'] > 0){
				if($info['retainage_payment'] != self::PAYMENT_ALIPAY){
					...
					$fre_res = $this->account->freezeRelease($seller,$info['seller_deposit'],$note,$info['user_id'],$seller,$info['order_no'],$info['amount']);
							...
							if($r1 === true){
								...
								if($r2 !== true){
									...
									if($r3 !== true){
										...
										$r4 = $reduce_amount > 0 ? $account_deposit->freezeRelease($buyer,$reduce_amount,$note,$buyer,$seller,$order['order_no'],$order['amount']) : true;
										$error = $r4 === true ? '' : $r4;
	...
	public function memberContractList($page,$where=''){
		...
		foreach ($data['list'] as $key => &$value) {
            ...
            $value['sub_mode'] = $product->getSubmode($value['sub_mode']);
	...
	public function contractReview($offer_id,$num,$user_id = ''){
		...
		$query->fields = 'po.type,po.sub_mode,po.mode,po.user_id as offer_user,po.price,po.price_l,po.price_vip,p.name,po.product_id,po.accept_area_code,po.accept_area,po.other,p.cate_id,p.produce_area,p.produce_address,p.unit';
		$query->where = 'po.id = :id';
		$query->bind = array('id'=>$offer_id);
		$res = $query->getObj();	
        $this->vipPrice($res);
	...
	public function contractDetail($id,$identity = 'buyer'){
		...
		$query->fields = 'do.*,po.type,po.other,po.price_vip,p.name,po.price,po.price_l,do.amount,p.unit,po.product_id,po.accept_area,po.accept_area_code,p.cate_id,p.img,p.produce_area,p.produce_address,pc.name as cate_name,po.user_id as seller_id';
