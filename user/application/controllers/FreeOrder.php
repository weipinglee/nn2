...
class FreeOrderController extends Yaf\Controller_Abstract{
	...
	public function buyerRetainageAction(){
		...
		$proof = 'xianxia.jpg';
		$res = $this->free->buyerRetainage($order_id,$user_id,$type,$proof);
		var_dump($res);
		return false;
	}
	...
	public function confirmProofAction(){
		...
		$res = $this->free->confirmProof($order_id,$user_id,$type);
		if($res['success'] == 1)
