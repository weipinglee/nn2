...
<div class="header">
	...
	<div class="user_body">
		<div class="user_b">
            ...
            <!--end左侧导航-->
            <div id="cont"><script type="text/javascript" src="/nn2/user/js/area/AreaData_min.js" ></script>
<script type="text/javascript" src="/nn2/user/js/area/Area.js" ></script>
			<!--start中间内容-->	
			<div class="user_c">
				<div class="user_zhxi">
					...
					<div class="chp_xx">
						<div class="xx_top">
							<form action="http://localhost/nn2/user/purchase/reportlists" method="GET" name="">
								<ul>
									<?php if(!isset($user_id)){?>
									<li>用户名：<input id="warename" name="name" type="text" value="<?php echo isset($name)?$name:"";?>"></li>
									<?php }?>
								
									<li>状态：<select name="status">
									<option value="9">全部</option>
										<?php if(!empty($status)) foreach($status as $key => $item){?>
											<option value="<?php echo isset($key)?$key:"";?>" <?php if( isset($s) && $key===$s){?>selected<?php }?>><?php echo isset($item)?$item:"";?></option>
										<?php }?>
									</select></li>
									<li>申请时间：<input class="Wdate" type="text" name="beginDate" value="<?php echo isset($beginDate)?$beginDate:"";?>" onclick="WdatePicker()"> <span style="position: relative;left: -3px;">—</span><input class="Wdate" type="text" name="endDate" value="<?php echo isset($endDate)?$endDate:"";?>" onclick="WdatePicker()">
									</li>
						...
						<div class="xx_center">
							<table border="0"  cellpadding="" cellspacing="">
								...
								<?php if(!empty($reportLists)) foreach($reportLists as $key => $list){?>
								<?php  $key++; ?>
								<tr>
									<td><?php echo isset($key)?$key:"";?></td>
									<td><p><?php echo isset($list['username'])?$list['username']:"";?></p></td>
									<td><?php echo isset($list['product_name'])?$list['product_name']:"";?></td>
									<td><span id="area<?php echo isset($key)?$key:"";?>"><?php $id='area'.$key; ?>                    <span id="areatext<?php echo $id ; ?>">
                        <script type="text/javascript">
                         ( function(){
                            var areatextObj = new Area();
                            var text = areatextObj.getAreaText('<?php echo $list['produce_area'] ; ?>',' ');
                            $('#areatext<?php echo $id ; ?>').html(text);
									...
									<!-- <td>已投保</td> -->
									<td><span class="col000000"><?php echo isset($list['status_zn'])?$list['status_zn']:"";?></span></td>
									<td><?php echo isset($list['create_time'])?$list['create_time']:"";?></td>
									<td>
										
										<?php if(!isset($user_id) && $list['status'] == \nainai\offer\product::OFFER_APPLY){?>
										<a href="http://localhost/nn2/user/purchaseorder/geneorder/id/<?php echo $list['id'];?>/offer_id/<?php echo $list['offer_id'];?>">选择</a>
										<?php }?>
									</td>
								</tr>
								<?php }?>
