
			<!--end左侧导航-->	
			<!--start中间内容-->	
			<div class="user_c">
				<div class="user_zhxi">
					<div class="zhxi_tit">
						<p><a>产品管理</a>><a>产品发布</a></p>
					</div>
					<div class="chp_xx">
						<div class="offer">
							<div class="offer_left">
								<img src="{views:/images/center/offter1.jpg}">
							</div>
							<p class="of_title1">
								<a class="title1_a" href="{url:/ManagerDeal/offer?mode=2}">保证金报盘</a>
							</p>
							<p class="of_title2">保证金报盘有什么吗</p>
							<p class="of_title1">保证金报盘优点</p>
						</div>
						<div class="offer">
							<div class="offer_center">
								<img src="{views:/images/center/offter2.jpg}">
							</div>
							<p class="of_title1">
								<a class="title1_a" href="{url:/ManagerDeal/offer?mode=1}">自由报盘</a>
							</p>
							<p class="of_title2">自由报盘有什么吗</p>
							<p class="of_title1">自由报盘有什么吗</p>
						</div>
						<div class="offer">
							<div class="offer_right">
								<img src="{views:/images/center/offter3.jpg}">
							</div>
							<p class="of_title1">
								<a class="title1_a" href="{url:/ManagerDeal/store?mode=3}">仓单报盘</a>
							</p>
							<p class="of_title2">仓单报盘有什么吗</p>
							<p class="of_title1">仓单报盘有什么吗</p>
						</div>
					 <div class="offer">
			                            <div class="offer_right">
			                                <img src="{views:/images/center/weituo.png}">
			                            </div>
			                            <p class="of_title1">
			                                <a class="title1_a" href="{url:/ManagerDeal/offer?mode=4}">委托报盘</a>
			                            </p>
			                            <p class="of_title2">委托报盘有什么吗</p>
			                            <p class="of_title1">委托报盘有什么吗</p>
			                        </div>
						
					</div>
				</div>
			</div>

